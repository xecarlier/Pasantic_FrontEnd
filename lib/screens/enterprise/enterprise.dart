...
class _EnterpriseState extends State<Enterprise> {
  ...
  CameraPosition? pos = CameraPosition(target: LatLng(-2.16,-79.89), zoom: 11.5);
  Marker? marca = Marker(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ready==false? Cargando() : SizedBox(
  ...
  Future<void> getEnterprInfo() async {
    try {
      final Response resEnterpriseData = await dioConst.get('$kUrl/organization/enterprise/${widget.id}/', 
                                    );
      ...
      final Response resContactData = await dioConst.get('$kUrl/user/agent/${enterpriseInfo!.contactId!}/', 
                                    );
      ...
      dynamic next = -1;
      while (next != null) {  
        final Response resReviewData = await dioConst.get('$kUrl/organization/review/', 
                                    );
